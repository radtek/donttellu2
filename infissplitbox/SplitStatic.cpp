...
#include "../gfimglib/include/gfimglib.h"

//extern int INFIS_CompressByWSQ(unsigned char *pSrc, int nw, int nh, int nCprRatio, unsigned char **ppDest, int *pDataLen);
...
#define STD_HEIGHT	640
#define STD_WIDTH	640
void CSplitStatic::GetSplittedFlatData(bool bWrite2File, int nImageType)
{
	_mapSplitedImg.clear();
	...
	for ( int i = 0; i < m_nMaxSplitFlat; ++i )
	{
		...
		{
			bool	bNeedPad = false;
			if (nw < STD_WIDTH || nw < STD_HEIGHT)
				bNeedPad = true;

			//补齐640 * 640
			if (bNeedPad)
			{
				int nOffsetH = 0, nOffsetV = 0;
				if (nh < STD_HEIGHT)
					nOffsetV = (STD_HEIGHT - nh) / 2;

				if (nw < STD_WIDTH)
					nOffsetH = (STD_WIDTH - nw) / 2;
				...
				char	*pDest = (char *)GAFIS_MALLOC(STD_HEIGHT * STD_WIDTH);
				char	*pSrc = (char *)pCuttedData;

				int		nGrayValue = 255;
				memset(pDest, nGrayValue, STD_HEIGHT * STD_WIDTH);

				for (int i = 0; i < STD_HEIGHT; ++i)
				{
					if (i < nOffsetV)
						;
					else if (i < nh + nOffsetV)
						memcpy(pDest + i * STD_WIDTH + nOffsetH, pSrc + (i - nOffsetV) * nw, nw);
					else
						;
				}

				AFIS_FREE(pCuttedData);
				pCuttedData = (UCHAR *)pDest;
			}

			std::string	strImage((char *)pCuttedData, STD_WIDTH * STD_HEIGHT);
			_mapSplitedImg[nFgp] = std::move(strImage);

#if 0
			CString	strTemp;
			strTemp.Format(_T("%d.bmp"), i + 1);
			GAFIS_SaveDataAsBMP(strTemp, pCuttedData, STD_WIDTH, STD_HEIGHT, 8);
#endif
		...
		else
		{
			...
			case HANDSPLITE_OBJTYPE_RLTHUMB:
				strTemp.Format(_T("rlthumb_%d.%s"), nFgp, pszType);
				break;
			case HANDSPLITE_OBJTYPE_RPLAINFOUR:
				strTemp.Format(_T("%s_four_%d.%s"), "r", nFgp, pszType);
				break;
			case HANDSPLITE_OBJTYPE_LPLAINFOUR:
				strTemp.Format(_T("%s_four_%d.%s"), "l", nFgp, pszType);
				break;
			}
			...
			if (nImageType == IMAGE_TYPE_WSQ)
			{
				...
				int				nDataLen = 0;
				if (0 > INFIS_CompressByWSQ(pCuttedData, nw, nh, 10, &pData, &nDataLen))
				{
					continue;
				}

				GAFIS_SaveBinDataToFile(strTemp, pData, nDataLen);
