...
namespace BLUEV {
	namespace BLUEVNET{
		...
		int	BLUEV_Server_Ex(void *pParam)
		{
			if(pParam == NULL)
				return -1;
			...
			ms._nIPAddr = pCtx->nIPAddr;
			ms._nPort = pCtx->nPort;
		...
		unsigned WINAPI	BLUEV_PreForkServer_Proc(void *pParam)
		{
			...
			do
			{
				...
				retval = BLUEV_Server_Ex(pContext);
		...
		unsigned WINAPI BLUEV_Server_Proc(void *pParam)
		{
			...
			TCPServerContext *pCtx = (TCPServerContext *)pParam;
			int retval = BLUEV_Server_Ex(pParam);
		...
		int TCPServerThread::run()
		{
			...
			while(!_svcStatus.isWillStop())
			{
				...
				pCtx->nIPAddr = (int)addrin.sin_addr.S_un.S_addr;
				pCtx->nPort = addrin.sin_port;
				pCtx->param = &_stParam;
		...
		{
			TCPServerThread	*pThread = (TCPServerThread *)pParam;
			if(pThread == NULL)
				return -1;
		...
		int TCPServer::close()
		{
			...
			{
				//唤醒线程池
				_queContext.bStop = true;

				//1个add thread， n个get thread
				if(_queContext.hGetDataEvent)
					SetEvent(_queContext.hGetDataEvent);
			
				for(int i = 0; i < _param.threadLimit; ++i)
				{
					if(_queContext.hAddDataEvent)
						SetEvent(_queContext.hAddDataEvent);
				}
			}*/
