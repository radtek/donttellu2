	...
	namespace BVP2PSVRLIB{
		...
		int UDPP2PService::proxyAnswer(UDPConnection& conn, UDPServiceParamContext& ctx)
		{
			...
			try
			{
				...
				//解析mainctrl节点
				if(0 > JsonParser::json2MainCtrl(root[JSON_MAINCTRL], llSessionId))
					throw BluevError(__FILE__, __LINE__);
			...
			try
			{
				...
				try
				{
					...
					else
					{
						//群发反馈，为了同步各主控端的设备状态。
						vct_users_t	vctUsers;
						if(0 > _ctxMgr.getUsers(vctUsers) || vctUsers.empty())
							throw BluevError(__FILE__, __LINE__, BluevError::ERR_PROXYANS);
						...
						for(itr = vctUsers.begin();
							...
							itr++)
						{
							const P2PUserData	&mainData = *itr;
							...
							//跳过自身
							if(mainData._llSessionId == devData._llSessionId)
								continue;
							...
							try
							{
								...
								std::cout << "代理同步转发应答从" << llDevSessionId << "至" << mainData._llSessionId << std::endl;
		...
		int JsonParser::json2MainCtrl(const Json::Value &mainctrl, __int64 &llSessionId) throw()
		{
			try
			{
		...
		int JsonBuilder::userData2Json(const std::string &strLanIp, const int &nLanPort,
			const std::string &strMask, bool bMainCtrl,
			const std::string &strId, std::string &strJson)
		{
			if(nLanPort <= 0)
				return -1;
			...
			try
			{
				...
				root[JSON_LANIP] = Json::Value(strLanIp);
				root[JSON_LANPORT] = Json::Value(nLanPort);
				root[JSON_MASK] = Json::Value(strMask);
				root[JSON_ISMAINCTRL] = Json::Value(bMainCtrl ? 1 : 0);
				if(bMainCtrl)
					root[JSON_MAC] = Json::Value(strId);
				else
					root[JSON_DEVID] = Json::Value(strId);
