...
public:
	...
	void SetPreesed(BOOL bPressed, BOOL bRepaint = TRUE);
	void SetDrawPressed(BOOL bDraw){ m_bDrawPressed = bDraw; };
	//void SetFillColor(COLORREF crNew, BOOL bRepaint = FALSE)
	//{
	//	m_crFillRect = crNew;
	//	if ( bRepaint ) Invalidate();
	//}
	//void SetFrameColor(COLORREF crNew, BOOL bRepaint = FALSE)
	//{
	//	m_crFrameRect = crNew;
	//	if ( bRepaint ) Invalidate();
	//}

	void SetBmpRes(CBmpResource *pBmResBtnNormal, CBmpResource *pBmResBtnPress);
protected:
    //{{AFX_MSG(CButtonST)
...
private:
	...
	//COLORREF  m_crFrameRect;
	//COLORREF  m_crFillRect;

	CBmpResource	*m_pBmResBtnPress;
	CBmpResource	*m_pBmResBtnNormal;
	BOOL			m_bDrawPressed;
};
