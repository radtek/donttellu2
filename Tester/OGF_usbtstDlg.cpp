...
#include "..\SDK_DLL\FingerSensor.h"
...
void COGF_usbtstDlg::CutImage()
{
	// TODO: 在此添加控件通知处理程序代码
#if 0
	CBTYStatus		&btyStatus = pPalmStatic->m_BtyStatus;
	if (!btyStatus.bSlap) return;

	GYTH_CAPTIMAGEDATA	*pstPalmData = btyStatus.pstCaptImage;

	//if (!m_pCtrl->m_ConfigInfo.IsCapturedDataValid(pstPalmData)) return;
	if (pstPalmData->pbmInfo->bmiHeader.biBitCount != CUtilHelper::BPP_8) return;

	REGIONRECT	stSrcObj = { 0 };
	REGIONRECT	stDesObj[4] = { 0 };
	int		nExistObj, nDesObj;
	int		retval;

	//	stSrcObj.nWidth  = pstPalmData->pbmInfo->bmiHeader.biWidth;
	...
	if (nDesObj < 1) return;
	//!< 都已经存在了，是不是就不用再切割一次了
	if (m_pCtrl->m_ConfigInfo.m_bNotSplitIfExist && (nDesObj == nExistObj)) return;
#endif
	REGIONRECT	stSrcObj = { 0 };
	REGIONRECT	stDesObj[4] = { 0 };
...
void COGF_usbtstDlg::OnSave() 
{
    ...
    static int no = 0;
    char filename[256];
    sprintf(filename, "fingerImg_%d.bmp", no);
	...
	GAFIS_SaveDataAsBMP(filename, imagebuff, nImgWidth, nImgHeight, 8);
