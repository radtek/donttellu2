...
namespace BLUEV
{
	namespace BVP2PSVRLIB
	{
		...
		public:
			...
			bool operator==(UserConnInfo &data)
			{
				return _llSessionId == data._llSessionId;
			}
		};

		typedef std::vector<UserConnInfo>	vct_userconn_t;
		class P2PUserData
		{
		public:
			__int64		_llSessionId;	//session id
			...
			vct_userconn_t	_vctMCtrl;	//控制本设备的主控端 sessionid
			vct_userconn_t	_vctUCtrl;	//本主控端控制的设备 sessionid
			...
			P2PUserData& operator=(const P2PUserData& data)
			{
				...
				//std::copy(data._vctUCtrl.begin(), data._vctUCtrl.end(), _vctUCtrl.begin());
		...
		public:
			...
			bool isLogin(const P2PUserData &userData, P2PUserData &queryData) const
			{
					...
					)
				{
					...
					{
						if(pUserData->_strMAC != userData._strMAC ||
							pUserData->_strUserName != userData._strUserName ||
							pUserData->_strPassWord != userData._strPassWord)
							continue;
					}
					else
					{
						if(pUserData->_strDeviceId != userData._strDeviceId)
							continue;
					}

					if(pUserData->_strNATIP != userData._strNATIP 
						|| pUserData->_nNATPort != userData._nNATPort
						|| pUserData->_strLANIP != userData._strLANIP
						|| pUserData->_nLANPort != userData._nLANPort
						|| pUserData->_strGateWay != userData._strGateWay
						|| pUserData->_bMainCtrl != userData._bMainCtrl
						)
					{
						continue;
					}
		...
		public:
			...
			int addUpdateConnInfo(__int64 llSessionId, __int64 llDevSessionId, ConnType enumType)
			{
				...
				map_users_t::iterator itr, itrDev;
				itr = _mapUsers.find(llSessionId);
				if(itr == _mapUsers.end())
					return -1;
				...
				P2PUserData & data = *itr->second;
				P2PUserData & devData = *itrDev->second;
		...
		private:
			int addUpdateVct(P2PUserData & data, __int64 llSessionId, ConnType enumType)
			{
				if(llSessionId <= 0)
					return -1;
		...
		class	CPSTUserCtxMgr
		{
		private:
			//std::map<std::string, xx>;
