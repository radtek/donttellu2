...
Finish_Exit:
	if(pCtx->nAnsLen > 0)
	{
		...
		uint2ToChar2(nOpClass, pAns->nOpClass);
...
int ProxyReq(UDPConnection *pConn, UDPServiceParam *pCtx)
{
	...
	//构造设备连接信息
	{
		...
		if(0 > RebuildConn("127.0.0.1", nPort, &addr, &nAddrLen))
		{
			if(pData)
				free(pData);
...
int CompAppInfoListNode2(AppInfoNode *pNode, AppInfoNode *pNodeComp)
{
	...
	if(0 == memcmp(pNode->pData->pszIp, pNodeComp->pData->pszIp, strlen(pNodeComp->pData->pszIp)))
		return 1;
